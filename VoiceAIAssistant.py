...
class VoiceAssistant:
    ...
    def record_audio(self, duration=None):
        ...
        sf.write(self.audio_file, recording, self.rate)
    ...
    def transcribe_audio(self, audio_file):
        ...
        try:
            # Fix: Check the correct parameter name for the ElevenLabs API
            with open(audio_file, 'rb') as f:
                result = self.eleven_client.speech_to_text.convert(
                    audio=f,  # Changed from audio_file to audio
    ...
    def speak_response(self, text):
        ...
        try:
            ...
            # Fallback to print the response when TTS fails
    ...
    def check_api_keys(self):
        """Check if the API keys are valid and not placeholders"""
        is_valid = True
        
        if self.elevenlabs_api_key == "YOUR_ELEVENLABS_API_KEY" or self.elevenlabs_api_key is None:
            logger.warning("ElevenLabs API key is not set. Speech functionalities will not work.")
            print("WARNING: ElevenLabs API key is not set. Please set it in the environment or in the code.")
            is_valid = False
            
        if self.nebius_api_key == "YOUR_NEBIUS_API_KEY" or self.nebius_api_key is None:
            logger.warning("Nebius API key is not set. AI responses will not work.")
            print("WARNING: Nebius API key is not set. Please set it in the environment or in the code.")
            is_valid = False
...
if __name__ == "__main__":
    ...
    keys_valid = assistant.check_api_keys()
    if not keys_valid:
        print("\nTo set environment variables:")
        print("  - On Linux/Mac: export ELEVENLABS_API_KEY=your_key_here")
        print("  - On Windows: set ELEVENLABS_API_KEY=your_key_here")
        print("\nOr edit the script to add your keys directly.")
        user_choice = input("\nContinue anyway? (y/n): ")
